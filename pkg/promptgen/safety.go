...
func BuildSafety(tools []ToolInfo) Section {
	...
	bullets := []string{
		...
		`- **System state changes between turns, in both directions.** Operators update policies, install skills, configure providers between your turns. A tool that was denied or missing earlier may be available now — and one you used before may be gone. Your tool list this turn answers either way: a memory of having used a tool is not evidence you still have it, and recalled output from one is a record of what was true then, not a reading of what is true now. When in doubt, attempt the call again` +
