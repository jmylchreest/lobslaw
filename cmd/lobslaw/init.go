...
func fillFromStdin(ans *initAnswers) error {
	...
	ans.NodeID = prompt(r, "node id", defaultNodeID())
...
func fillFromEnv(ans *initAnswers) error {
	ans.NodeID = envOr("LOBSLAW_INIT_NODE_ID", defaultNodeID())
...
// defaultNodeID returns a stable-ish default based on hostname, or a
// random 6-char hex fallback when hostname is unavailable.
