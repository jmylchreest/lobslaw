...
`*.pem` is the one pattern where the extension does not say what the file is. PEM is a container — a private key is PEM, and so is the certificate that key signed, which is the public half and is meant to be read. A name that unambiguously reads as a certificate (`ca.pem`, `fullchain.pem`, `node-cert.pem`) downgrades to **confirm**; anything mentioning `key`, `priv` or `secret` stays denied whatever else it says, and a name matching neither list stays denied too. The default is the floor, not the exception.

One asymmetry falls out of that and is deliberate: `CheckCommandPaths` refuses a *confirm* verdict outright, because a shell has nowhere to ask mid-command. So `ca.pem` becomes readable via `read_file` and stays refused inside a `shell_command`. Pinned by a test, so nobody later "fixes" it into a silent allow.
