...
// as CrossOwnerAuthorizer, and it keeps the builtins testable without a
// workspace.
//
...
func newSlackReadHandler(r SlackReader) BuiltinFunc {
...
func newSlackSearchHandler(r SlackReader) BuiltinFunc {
	return func(ctx context.Context, args map[string]string) ([]byte, int, error) {
	...
	return []*types.ToolDef{
		{
			...
			Path:        BuiltinScheme + "slack_read_channel",
		...
		{
			Name:        "slack_search",
			Path:        BuiltinScheme + "slack_search",
