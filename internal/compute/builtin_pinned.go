...
func pinnedWriteHandler(store PinnedMemoryStore, failures *pinnedFailures) BuiltinFunc {
...
func pinnedForgetHandler(store PinnedMemoryStore, failures *pinnedFailures) BuiltinFunc {
	return func(ctx context.Context, args map[string]string) ([]byte, int, error) {
		turnID, userID, kind, err := pinnedContext(ctx, args)
		if err != nil {
			return nil, 2, err
	...
	return []*types.ToolDef{
		{
			...
			Path:        BuiltinScheme + "pinned_remember",
		...
		{
			Name:        "pinned_forget",
			Path:        BuiltinScheme + "pinned_forget",
