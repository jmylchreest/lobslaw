...
func NewLLMClient(cfg LLMClientConfig) (*LLMClient, error) {
	...
	if hc == nil {
		...
		// proxy under role "llm" (broad — every provider host the
		// operator declared in [[compute.providers]]). Per-provider
		// roles "llm/<label>" are also generated by the ACL builder
		// for callers that want to lock down a specific client; not
