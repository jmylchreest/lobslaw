...
func RegisterBotBuiltins(b *Builtins, cfg BotConfig) error {
	...
	return b.Register("ask_bot", newAskBotHandler(cfg.Runner, cfg.Resolver, cfg.Inbox))
}
...
func newAskBotHandler(runner *compute.TurnRunner, bots compute.BotResolver, inbox InboxService) compute.BuiltinFunc {
	return func(ctx context.Context, args map[string]string) ([]byte, int, error) {
		...
		if err != nil {
			// Fails closed and says who refused. A child that tripped a
			// confirmation cannot ask for one — the person is in a
			// conversation with the CALLER, and R2's prompt carries
			// RaisedFor precisely so the wrong party cannot answer.
