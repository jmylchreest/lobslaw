...
func (f *fakeBrowser) Read(_ context.Context, key turn.SessionKey, fromSeq uint64, limit int) ([]compute.Message, error) {
	f.lastFrom = fromSeq
	f.lastLimit = limit
	f.lastKey = key
...
func TestSessionToolDefsAreWellFormed(t *testing.T) {
	...
	for _, d := range defs {
		...
		if !strings.HasPrefix(d.Path, compute.BuiltinScheme) {
