...
type CouncilConfig struct {
	Registry *compute.ProviderRegistry
...
func newCouncilReviewHandler(reg *compute.ProviderRegistry) compute.BuiltinFunc {
	return func(ctx context.Context, args map[string]string) ([]byte, int, error) {
		...
		var targets []compute.ProviderEntry
		...
		for i, t := range targets {
			...
			go func(i int, t compute.ProviderEntry) {
				...
				responses[i].Content = compute.StripReasoningTags(resp.Content)
			}(i, t)
		}
		wg.Wait()
		...
		if mode == "adversarial" {
			...
			for i, t := range targets {
				...
				go func(i int, t compute.ProviderEntry) {
					...
					round2[i].Content = compute.StripReasoningTags(resp.Content)
				}(i, t)
			}
			wg2.Wait()
			responses = round2
