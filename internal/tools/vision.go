...
func RegisterVisionBuiltin(b *Builtins, cfgs ...VisionConfig) error {
	...
	return b.Register("read_image", compute.FailoverBuiltin("read_image", nil, b.Health(), b.TrustFloor(), handlers...))
}
...
func VisionToolDef() *types.ToolDef {
	return &types.ToolDef{
		Name:        "read_image",
		Path:        compute.BuiltinScheme + "read_image",
...
func newReadImageHandler(cfg VisionConfig, client *http.Client) compute.BuiltinFunc {
	return func(ctx context.Context, args map[string]string) ([]byte, int, error) {
		...
		content, err := cfg.Driver.Describe(ctx, compute.VisionRequest{
			Question: question,
			MIME:     mime,
			Data:     data,
		})
