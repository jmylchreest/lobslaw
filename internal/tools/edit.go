...
func WriteToolDef() *types.ToolDef {
	return &types.ToolDef{
		Name:        "write_file",
		Path:        compute.BuiltinScheme + "write_file",
...
func EditToolDef() *types.ToolDef {
	return &types.ToolDef{
		Name:        "edit_file",
		Path:        compute.BuiltinScheme + "edit_file",
...
func writeFileBuiltin(_ context.Context, args map[string]string) ([]byte, int, error) {
	...
	if path == "" {
		return compute.MarshalToolError("missing_arg", "path is required", "pass an absolute filesystem path")
	}
	...
	if !filepath.IsAbs(path) {
		return compute.MarshalToolError("relative_path", "path must be absolute OR mount-scoped",
			"use '/abs/path' or 'mount-label/subpath' where the mount is writable")
	}
	if payload, exit, refused := compute.HardlinePathRefusal(path, "written"); refused {
		return payload, exit, nil
	}
	content := args["content"]
	if len(content) > writeFileMaxBytes {
		return compute.MarshalToolError("content_too_large",
	...
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		if os.IsNotExist(err) {
			return compute.MarshalToolError("parent_not_found", err.Error(),
				"the parent directory doesn't exist. Create it first or pick an existing directory")
		}
		if os.IsPermission(err) {
			return compute.MarshalToolError("permission_denied", err.Error(), "")
		}
		return compute.MarshalToolError("write_failed", err.Error(), "")
	}
...
func editFileBuiltin(_ context.Context, args map[string]string) ([]byte, int, error) {
	...
	if !filepath.IsAbs(path) {
		return compute.MarshalToolError("relative_path", "path must be absolute OR mount-scoped", "use '/abs/path' or 'mount-label/subpath' where the mount is writable")
	}
	if payload, exit, refused := compute.HardlinePathRefusal(path, "edited"); refused {
		return payload, exit, nil
	}
	oldStr := args["old_string"]
	newStr := args["new_string"]
	if oldStr == "" {
		return compute.MarshalToolError("missing_arg", "old_string is required",
			"pass the exact substring to replace")
	}
	if oldStr == newStr {
		return compute.MarshalToolError("noop_edit", "old_string equals new_string; nothing to change", "")
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return compute.MarshalToolError("path_not_found", path+" does not exist",
				"use write_file to create a new file, or pick an existing path")
		}
		return compute.MarshalToolError("read_failed", err.Error(), "")
	}
	bodyStr := string(body)
	count := strings.Count(bodyStr, oldStr)
	if count == 0 {
		return compute.MarshalToolError("old_string_not_found",
	...
	if !replaceAll && count > 1 {
		return compute.MarshalToolError("ambiguous_match",
