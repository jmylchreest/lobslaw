...
func DebugToolDefs() []*types.ToolDef {
	...
	return []*types.ToolDef{
		...
		mk("debug_version", "Return the running build's version, commit and build time, plus node_id and enabled functions. This is the only source for what version this node is running; never state a version it did not report. A DIRTY marker means the binary was built from a tree with uncommitted changes, so the commit names the source only approximately. Takes no arguments."),
...
func wrapDebug(fn func() any) compute.BuiltinFunc {
...
func wrapDebugString(fn func() string) compute.BuiltinFunc {
	return func(_ context.Context, _ map[string]string) ([]byte, int, error) {
