...
func DebugToolDefs() []*types.ToolDef {
	...
	return []*types.ToolDef{
		...
		mk("debug_version", "Return the running build's version and commit, plus node_id and enabled functions. This is the only source for what version this node is running; never state a version it did not report. Takes no arguments."),
