...
func readFileBuiltin(_ context.Context, args map[string]string) ([]byte, int, error) {
	...
	if err != nil {
		...
		if os.IsPermission(err) {
			return compute.MarshalToolError("permission_denied", path+": permission denied",
				"the lobslaw process cannot read this file; try a path inside a configured storage mount")
		}
		return compute.MarshalToolError("read_failed", "open "+path+": "+err.Error(),
			"check the path exists and is readable")
	}
	defer func() { _ = f.Close() }()

	if fi, statErr := f.Stat(); statErr == nil && fi.IsDir() {
		return compute.MarshalToolError("is_directory", path+" is a directory, not a file",
...
func listFilesBuiltin(_ context.Context, args map[string]string) ([]byte, int, error) {
	...
	if payload, exit, refused := compute.HardlinePathRefusal(path, "listed"); refused {
		return payload, exit, nil
	}
	...
	if !info.IsDir() {
		return compute.MarshalToolError("is_file", path+" is a file, not a directory",
...
func globBuiltin(ctx context.Context, args map[string]string) ([]byte, int, error) {
	...
	if pattern == "" {
		return compute.MarshalToolError("missing_arg", "pattern is required",
	...
	if payload, exit, refused := compute.HardlinePathRefusal(root, "walked"); refused {
		return payload, exit, nil
	}
	if _, err := os.Stat(root); err != nil {
		if os.IsNotExist(err) {
			return compute.MarshalToolError("path_not_found", root+" does not exist",
...
func searchFilesBuiltin(ctx context.Context, args map[string]string) ([]byte, int, error) {
	...
	if _, err := os.Stat(searchPath); err != nil {
		if os.IsNotExist(err) {
			return compute.MarshalToolError("path_not_found", searchPath+" does not exist",
	...
	if err != nil {
		...
		return compute.MarshalToolError("search_failed", stderr,
