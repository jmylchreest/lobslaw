...
type SessionBrowser interface {
	...
	Read(ctx context.Context, key turn.SessionKey, fromSeq uint64, limit int) ([]compute.Message, error)
...
// reach it. compute.Agent.runLoop attaches a scope unconditionally — including
...
func newSessionSearchHandler(cfg SessionToolConfig) compute.BuiltinFunc {
...
func newSessionReadHandler(cfg SessionToolConfig) compute.BuiltinFunc {
	return func(ctx context.Context, args map[string]string) ([]byte, int, error) {
		channel := strings.TrimSpace(args["channel"])
		...
		for _, m := range msgs {
			b.WriteString(compute.RenderForSummary(m, compute.DefaultCompactToolResultBytes))
		}
		return []byte(b.String()), 0, nil
	}
}
...
func SessionToolDefs() []*types.ToolDef {
	return []*types.ToolDef{
		{
			Name:        "session_search",
			Path:        compute.BuiltinScheme + "session_search",
		...
		{
			Name:        "session_read",
			Path:        compute.BuiltinScheme + "session_read",
