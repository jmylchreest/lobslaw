...
type Builtins struct {
	mu       sync.RWMutex
	handlers map[string]compute.BuiltinFunc
...
func NewBuiltins() *Builtins {
	return &Builtins{handlers: make(map[string]compute.BuiltinFunc)}
}
...
func (b *Builtins) SetHealth(h *compute.ProviderHealth) {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.health = h
}
...
func (b *Builtins) Health() *compute.ProviderHealth {
	if b == nil {
		return nil
	}
	b.mu.RLock()
...
func (b *Builtins) Register(name string, fn compute.BuiltinFunc) error {
	if name == "" {
...
func (b *Builtins) Get(name string) (compute.BuiltinFunc, bool) {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func isBuiltinPath(path string) (string, bool) {
	if !strings.HasPrefix(path, compute.BuiltinScheme) {
		return "", false
	}
	return strings.TrimPrefix(path, compute.BuiltinScheme), true
}
