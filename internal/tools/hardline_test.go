...
func hardlineEnv(t *testing.T) *agentEnv {
	t.Helper()
	env := newAgentEnv(t)
	b := NewBuiltins()
...
func TestSessionGrantCannotReachTheFloor(t *testing.T) {
	...
	_, err := env.executor.Invoke(ctx, compute.InvokeRequest{
		ToolName: "run",
...
func TestSSHConfigPromptsRatherThanRefuses(t *testing.T) {
	...
	if !policy.IsHardline(err) || errors.Is(err, compute.ErrRequireConfirm) {
