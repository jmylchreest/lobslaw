...
func TestExecutorDispatchesBuiltin(t *testing.T) {
	...
	_ = env.reg.Register(&types.ToolDef{
		Name:     "echo",
		Path:     compute.BuiltinScheme + "echo",
		RiskTier: types.RiskReversible,
	})

	res, err := env.executor.Invoke(context.Background(), compute.InvokeRequest{
		ToolName: "echo",
...
func TestExecutorBuiltinErrorGoesToStderr(t *testing.T) {
	...
	env.executor.SetBuiltins(b)
	_ = env.reg.Register(&types.ToolDef{Name: "boom", Path: compute.BuiltinScheme + "boom", RiskTier: types.RiskReversible})
...
func TestExecutorBuiltinRejectedWhenRegistryMissing(t *testing.T) {
	t.Parallel()
	env := newAgentEnv(t)
	_ = env.reg.Register(&types.ToolDef{Name: "x", Path: compute.BuiltinScheme + "x", RiskTier: types.RiskReversible})

	// No SetBuiltins → Invoke should fail with compute.ErrToolPathInvalid.
	_, err := env.executor.Invoke(context.Background(), compute.InvokeRequest{
		ToolName: "x",
		Claims:   &types.Claims{Scope: "test"},
		TurnID:   "t",
	})
	if !errors.Is(err, compute.ErrToolPathInvalid) {
		t.Errorf("want compute.ErrToolPathInvalid; got %v", err)
	}
}
