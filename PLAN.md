...
- **8d Plugin install CLI.** Shipped. `lobslaw plugin {install,uninstall,list,enable,disable,import}`. Install = filesystem copy of a local directory with plugin.yaml + manifest-tree approval prompt (y/N/d). Enable/disable = `.disabled` marker file under the plugin dir (simpler than a separate registry; operators can flip state with `touch`/`rm`). Import = install from a Claude-Code-shape plugin dir (plugin.yaml path only; plugin.json → plugin.yaml conversion deferred). Git URL / clawhub ref resolution rejected with ErrUnsupportedSource pointing at the deferred work. Destination root honours `LOBSLAW_SKILLS_ROOT`, defaults to `~/.local/share/lobslaw/plugins`.
- **8e MCP client.** Shipped. `internal/mcp` implements JSON-RPC 2.0 over stdio (`initialize` / `tools/list` / `tools/call`), a `.mcp.json` manifest reader matching the Claude Code format, and a `Loader` that satisfies `compute.SkillDispatcher` so MCP-sourced tools appear in the agent's dispatch table interchangeably with local skills. `secret_env` entries resolve through the shared resolver. Transport is abstracted so unit tests use an in-memory fake; production uses `StdioTransport` over `exec.Cmd`. Streaming, resources, prompts, sampling, and server-push notifications deferred.
